...
function statusText(status) {
  if (status === "failed") return "Платёж отклонён платёжной системой.";
...
export default function PaymentFailPage({ onToggleTheme, onOpenPurchase }) {
  ...
  useEffect(() => {
    ...
    function scheduleRetry(retryAfterSeconds, message) {
      ...
      setState({
        ...
        message: shouldStopAfterCountdown
          ? "Ждём подтверждение до 3 минут. Если оно не придёт, покажем инструкцию для поддержки."
          : message,
      ...
      intervalId = window.setInterval(() => {
        countdown -= 1;
        setState((current) =>
          current.phase === "waiting" ? { ...current, nextRetrySeconds: Math.max(0, countdown) } : current,
        );
      }, 1000);
    ...
    async function checkPayment() {
      ...
      try {
        ...
        scheduleRetry(result.retryAfterSeconds, "Robokassa ещё не прислала подтверждение. Проверим автоматически.");
      } catch (error) {
        if (isCancelled) return;
        ...
        setState({
          ...
          invoice,
          orderId,
          message: error.message || "Не удалось проверить платёж.",
          nextRetrySeconds: 0,
          amountRub: invoice.amount_rub || outSum || "",
          topupCode: "",
        });
  ...
  const isFinalFailure = ["missing", "terminal", "timeout", "error"].includes(state.phase);
        ...
        <section className="container payment-hero" aria-labelledby="payment-fail-title">
          <div className="payment-copy">
            ...
            <h1 id="payment-fail-title" className="payment-title">
              {state.phase === "paid" ? "Пополнение подтверждено." : isFinalFailure ? "Платёж не подтверждён." : "Проверяем платёж."}
            </h1>
            <p>{state.message}</p>
            {state.phase === "waiting" ? (
          ...
          <div className="payment-panel" aria-hidden="true">
            <div className="payment-panel-status">{state.phase === "paid" ? "PAID" : isFinalFailure ? "FAIL" : "CHECK"}</div>
            <div className="payment-panel-card">
              <span>{state.phase === "paid" ? "Код готов" : isFinalFailure ? "Платёж не подтверждён" : "Серверная проверка"}</span>
